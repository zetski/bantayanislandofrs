...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM reports WHERE municipality = ? AND barangay = ? AND sitio_street = ?");
    ...
    if ($count > 0) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($fileTmpPath, $destPath)) {
                ...
                $stmt = $conn->prepare("INSERT INTO reports (lastname, firstname, middlename, contact, subject, message, municipality, barangay, sitio_street, image_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
